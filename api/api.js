...
async function loadProjectsForAutoFlow() {
  ...
  try {
    ...
    // Fetch projects for EACH unique date in worklogs - the API returns projects
    // available per date, so using only start date can miss projects allocated
    // on other dates (causing them to fall back to "Other" with id 0)
    const uniqueDates = [...new Set(AppState.autoGeneratedWorklogs.map(wl => wl.date))];
    try {
      const projectFetchPromises = uniqueDates.map((date) =>
        fetch(
          `https://sra-api.smartosc.com/api/projects/all?userId=${AppState.currentUserId}&date=${date}`,
          {
            headers: {
              accept: "application/json, text/plain, */*",
              authorization: `Bearer ${AppState.currentToken}`,
      ...
      const projectResponses = await Promise.all(projectFetchPromises);
      ...
      // Failed to fetch project IDs
    }

    // Update worklogs with project IDs (try exact match, then case-insensitive)
    AppState.autoGeneratedWorklogs.forEach((wl) => {
      let projectId = projectCodeToIdMap.get(wl.projectCode);
      if (projectId === undefined && wl.projectCode) {
        const codeUpper = wl.projectCode.toUpperCase();
        for (const [k, v] of projectCodeToIdMap.entries()) {
          if (k && k.toUpperCase() === codeUpper) {
            projectId = v;
            break;
          }
        }
      }
      wl.projectId = projectId ?? 0; // Use 0 (Other) if not found
    });
...
async function submitWorklog() {
  ...
  if (autoCheckboxes.length > 0) {
    ...
    workLogs = checkedEntries.reduce((acc, checkbox) => {
      ...
      if (hours <= 0 || hours > 24) {
        skippedDates.push(worklog.date);
        return acc;
      }
  ...
  // Skip days where total hours across all entries for that day exceeds 24
  const dailyTotals = {};
  workLogs.forEach(wl => {
    dailyTotals[wl.date] = (dailyTotals[wl.date] || 0) + wl.workHours;
  });
  const overloadedDates = new Set(
    Object.keys(dailyTotals).filter(date => dailyTotals[date] > 24)
  );
  if (overloadedDates.size > 0) {
    overloadedDates.forEach(date => {
      if (!skippedDates.includes(date)) skippedDates.push(date);
    });
    workLogs = workLogs.filter(wl => !overloadedDates.has(wl.date));
  }

  if (workLogs.length === 0) {
    alert(
      skippedDates.length > 0
        ? `No valid entries to submit. All selected days were skipped because their total hours exceed 24h:\n${skippedDates.join(", ")}`
  ...
  try {
    ...
    let successMessage = result.message || "Worklog submitted successfully";
    if (skippedDates.length > 0) {
      successMessage += `\n\nNote: ${skippedDates.length} day(s) were skipped because total hours exceeded 24h: ${skippedDates.join(", ")}`;
    }
...
async function submitQuickBooking(projectId, userBookingData) {
  try {
    // Ensure userBookingData is an array
    if (!Array.isArray(userBookingData)) {
      throw new Error("userBookingData must be an array");
    }
      ...
      quickBookingData: [
        {
          ...
          userBookingData: userBookingData, // Array of user booking objects
        },
      ],
    };

    console.log('=== API CALL: submitQuickBooking ===');
    console.log('Payload:', JSON.stringify(payload, null, 2));

    const response = await fetch(
      "https://sra-api.smartosc.com/api/projects/quick-booking",
      {
        method: "POST",
        headers: {
          accept: "application/json, text/plain, */*",
    ...
    if (responseText && responseText.trim() !== "") {
      ...
      // Empty response means success (204 No Content or similar)
      result = { message: "Quick booking submitted successfully" };
    }
